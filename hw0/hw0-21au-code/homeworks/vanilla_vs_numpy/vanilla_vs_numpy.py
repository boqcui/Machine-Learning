...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    a = vanilla_matmul(A, x)
    b = vanilla_matmul(vanilla_transpose(A), x)
    c = vanilla_matmul(vanilla_transpose(B), y)
...
def numpy_solution(
    ...
    """
    return np.matmul(A,x) + np.matmul(A.transpose(),x) + np.matmul(B.transpose(),y)
